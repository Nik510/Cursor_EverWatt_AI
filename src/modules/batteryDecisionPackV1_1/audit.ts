...
import { roundTo, safeNum, stableSortById } from '../batteryEconomicsV1/helpers';

import { auditBoundsV1_1 } from './constants';
import type { BatteryDecisionPackAuditV1_1 } from './types';

function absAmount(li: BatteryEconomicsAuditLineItemV1): number {
  const raw = safeNum((li as any)?.amountUsdRaw) ?? safeNum((li as any)?.amountUsd);
...
function boundedLineItems(lineItems: BatteryEconomicsAuditLineItemV1[]): BatteryEconomicsAuditLineItemV1[] {
  ...
  for (const id of auditBoundsV1_1.alwaysIncludeIds) if (byId.has(id)) keep.add(id);
  ...
  for (const li of remaining) {
    if (keep.size >= auditBoundsV1_1.maxLineItems) break;
...
function reconcileFromLineItems(lineItems: BatteryEconomicsAuditLineItemV1[]): BatteryDecisionPackAuditV1_1['reconcile'] {
  const byId = new Map<string, BatteryEconomicsAuditLineItemV1>();
  ...
  const total = safeNum(byId.get('savings.totalAnnual')?.amountUsd);
...
export function buildBoundedAuditV1_1(args: { economicsAuditLineItems: BatteryEconomicsAuditLineItemV1[] | null | undefined }): BatteryDecisionPackAuditV1_1 {
