...
function dedupeByTimestampKeepFirst(points: IntervalElectricPointV1[]): { points: IntervalElectricPointV1[]; droppedCount: number; sample: string[] } {
...
export function parseIntervalElectricCsvV1(args: {
  ...
  // Prefer first meter deterministically (sorted by meterKey).
  ...
  // No silent partial parses: if upstream parser reported unparseable timestamps, fail intake with explicit reason code.
  ...
  // Duplicate timestamp guardrail: warn + deterministic dedupe policy (keep first occurrence).
  ...
  // Granularity guardrail: warn when mixed intervalMinutes values appear.
  ...
  const gapInfo =
    Number.isFinite(inferredIntervalMinutes) && Number(inferredIntervalMinutes) > 0
      ...
      : null;
  const gapWarning =
    gapInfo && gapInfo.gapsCount > 0
      ? [{ code: IntervalElectricIngestReasonCodesV1.INTERVAL_LARGE_GAPS, details: gapInfo }]
      : [];
