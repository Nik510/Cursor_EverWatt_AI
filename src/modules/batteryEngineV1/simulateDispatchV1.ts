...
function findPeakAndOffpeakPrices(args: { tariff: TariffPriceSignalsV1 | null | undefined }): {
  ...
  const t = args.tariff;
  const winsGen = Array.isArray((t as any)?.generationTouEnergyPrices) ? (((t as any).generationTouEnergyPrices as any[]) || []) : [];
  const winsDelivery = Array.isArray(t?.touEnergyPrices) ? (t!.touEnergyPrices as any[]) : [];
  const wins = winsGen.length ? (winsGen as any) : (winsDelivery as any);
