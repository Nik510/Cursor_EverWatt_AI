...
function parseUsDateTimeToUtcIso(args: { raw: string; timeZone: string }): { iso: string | null; because: string } {
  ...
  if (!Number.isFinite(month) || !Number.isFinite(day) || !Number.isFinite(year) || !Number.isFinite(hour) || !Number.isFinite(minute) || !Number.isFinite(second)) {
...
export function parsePgeIntervalCsvV1(args: {
  ...
  // Event Flags tolerated; not used.
